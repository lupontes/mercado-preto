...
- `GET /store/reviews/invite/{token}` — decodifica o token, busca o pedido e seus itens, retorna a lista de produtos avaliáveis daquele pedido (título, thumbnail, `product_id`) e quais já foram avaliados (pra desabilitar na tela, não esconder — feedback claro de "você já avaliou este"). Token inválido/expirado → `400` com mensagem clara.
