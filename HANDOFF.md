...
- [x] **Corrigidos os 3 achados secundários (não relacionados ao split em si).** Commits `fb775d4` + `b439d3a` (a primeira tentativa de `seller/orders/route.ts` quebrou a rota com 500 — "Shipping method version is required to load adjustments", um bug do Medusa ao pedir o campo decorado `total` numa consulta de lista para pedidos criados via `createOrders()` fora do fluxo completo de carrinho; corrigido calculando o total manualmente a partir de itens+frete). Os três, verificados no servidor de teste via browser:
  - `/painel/pedidos`: Total/Status/Data agora aparecem corretos (antes vinham em branco/"Invalid Date").
  - `/painel/perfil` (e qualquer tela com formulário longo): botão "Sair" agora fica sempre visível — barra lateral do painel virou `sticky` no desktop.
  - `/loja/[id]`: produto recém-publicado aparece na hora — removido o cache padrão de 60s do `apiFetch` especificamente pra `getSellerProducts`.
- [x] **Consolidação de WhatsApp confirmada empiricamente.** Commit `776bb6f` — adicionado `logger.info` em `order-placed-whatsapp.ts` logo antes do envio (independente de credenciais da Evolution API, que não estão configuradas neste ambiente). Reteste real (payment `1328050558`, pedidos #19 e #20): log mostrou **uma única linha** "mensagem preparada... pedido(s) #19 e #20, total R$ 207,25" — não duas linhas com totais parciais. Ver `docs/qa/2026-08-27-multi-seller-order-split-verification.md`, seção "Reteste 4".
- [ ] **PRÓXIMO PASSO:** branch pronta pro GitFlow (`superpowers:finishing-a-development-branch` — decidir integração PR `fix/...` → `develop` → `alpha` → `beta` → `release` → `main`). Pendências não-bloqueantes que ainda restam, pra tratar como issues separadas depois: (a) tela de confirmação do checkout ainda lê `payment.metadata` (que a própria spec desta branch provou vir vazio na maioria dos casos) — a persistência de snapshot já resolveu o lado backend, falta o lado storefront; (b) preço/total do item vêm 100% do cliente sem checagem contra o preço real, agora alimentando comissão e NF-e de verdade.
...
- [x] **Manual de teste pro Ailton** publicado como artifact (roteiro passo a passo, explicações didáticas, mapa de pendências) — https://claude.ai/code/artifact/67edf443-6586-4889-8d27-f82aee4f21fb. E-mail com o link fica como rascunho no Gmail até confirmação de envio.
