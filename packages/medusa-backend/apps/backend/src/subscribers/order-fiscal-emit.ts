...
export default async function orderFiscalEmit({
  ...
  // Pedidos do canal Mercado Livre (metadata.channel === "mercado_livre",
  // criados pelo webhook em src/api/webhooks/mercadolivre/route.ts) recebem
  // um shipping_address real, buscado via GET /shipments/:id na hora da
  // criação do pedido — mas essa busca é melhor-esforço (se a API do ML
  // falhar naquele momento, o pedido ainda é criado sem endereço). Por isso
  // os fallbacks abaixo ("Não informado", o CEP/cidade/UF padrão da própria
  // plataforma em Cachoeira/BA etc.) continuam necessários como rede de
  // segurança, não como comportamento normal esperado.
