...
export default async function commissionOnPayment({
  ...
  if (channel === "mercado_livre") {
    // Pedidos do ML nunca têm shipping_methods (o frete é 100% gerido pelo
    // Mercado Envios, fora deste sistema) — productsGross aqui já é o valor
    // total do pedido, respeitando a regra de que comissão nunca incide
    // sobre frete sem precisar de nenhuma lógica adicional de separação.
    ...
    bankingFees = Math.round(productsGross * (feePercent / 100) + feeFixed)
  } else {
    ...
    bankingFees = Math.round(productsGross * BANKING_FEE_PERCENT) + fixedFeeShare
  }
